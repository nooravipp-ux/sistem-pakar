...
<main class="batas-atas">
        <div class="card text-dark bg-white mb-3">
            <h5 class="card-header bg-white">EDIT DATA PASAL</h5>
                <div class="card-body text-left">

                <?php 
                    include('../database/koneksi.php');
                    $kd_pasal = $_GET['kd_pasal'];
                    $query = "SELECT * FROM tbl_pasal WHERE kd_pasal='$kd_pasal'";
                    if ($hasil = mysqli_query($koneksi, $query)){
                        while($row = mysqli_fetch_array($hasil, MYSQLI_ASSOC)){
                ?>
                <form action="proses-edit-pasal.php" method="POST" >
                    <div class="form-group row">
                        <label for="nama_pelaku" class="col-sm-2 col-form-label">KODE PASAL</label>
                        <div class="col-sm-5">
                            <input type="text" class="form-control" name="kd_pasal" value="<?php echo $row['kd_pasal'];?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="nama_pelaku" class="col-sm-2 col-form-label">HUKUMAN</label>
                        <div class="col-sm-5">
                        <input type="text" class="form-control" name="hukuman" value="<?php echo $row['hukuman'];?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="nama_pelaku" class="col-sm-2 col-form-label">KETETRANGAN</label>
                        <div class="col-sm-5">
                        <textarea id="form7" class="md-textarea form-control" rows="3" name="keterangan"><?php echo $row['keterangan'];?></textarea>
                        </div>
                    </div>
                        <?php }
                    } ?>
